...
export const ShoppingList: React.FC<ShoppingListProps> = ({
  ...
  const [newItemCategory, setNewItemCategory] =
    useState<ShoppingCategory>("Outros");
  ...
  const getStockStatusForProduct = (productName: string) => {
    ...
    const invItem = inventoryList.find(
      (item) => item.name.trim().toLowerCase() === nameLower
    );
    ...
    if (persistedMonths >= 2 && currentQty > 0) {
      return {
        ...
        message: "Persiste há 2+ meses. Comprar menor %.",
        suggestedQty: Math.max(1, Math.round(currentQty * 0.7 * 10) / 10),
      ...
      return {
        ...
        message: "Estoque abaixo do mínimo ideal.",
        unit: invItem.unit || "un",
        currentQty
      };
    } else {
  ...
  const handleQuickRegister = async () => {
    ...
    if (onAddRegisteredProduct) {
      try {
        const category = newItemCategory || "Outros";
        const unit = newItemUnit || "un";
        ...
        // Set the product fields for the active form
        setNewItemName(trimmed);
        setProductSearch(trimmed);
        setShowSuggestions(false);
      } catch (err) {
        console.error("Erro ao cadastrar produto rapidamente:", err);
      }
    } else {
      // Fallback
      setNewItemName(trimmed);
  ...
  const filteredSuggestions = useMemo(() => {
    ...
    if (!query) {
      // If query is empty, show the first 10 registered products
      return registeredProducts.slice(0, 10);
    }
  ...
  }, [registeredProducts, productSearch]);
  ...
  const handleRepeatNextMonth = () => {
    if (
      window.confirm(
        "Copiar os itens deste mês para o mês seguinte? O valor real atual será salvo como preço de referência no próximo.",
      )
    ) {
      ...
      filteredItems.forEach((item) => {
        onAdd({
          ...
          actualPrice: 0,
          referencePrice:
            item.actualPrice > 0 ? item.actualPrice : item.referencePrice,
          isChecked: false,
  ...
  const handleAddItem = async (e: React.FormEvent) => {
    ...
    if (!nameToUse && productSearch.trim()) {
      const trimmed = productSearch.trim();
      if (onAddRegisteredProduct) {
        try {
          const category = newItemCategory || "Outros";
          ...
          console.error("Erro ao cadastrar produto na submissão:", err);
        }
      }
      nameToUse = trimmed;
    }
  ...
  const budgetPercentage =
    shoppingBudget > 0 ? (forecastTotal / shoppingBudget) * 100 : 0;
          ...
          <div className="bg-white dark:bg-slate-800 rounded-xl shadow-2xl w-full max-w-md p-6 border border-purple-200 dark:border-purple-800 animate-scale-in">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-bold text-slate-800 dark:text-white flex items-center gap-2">
                <Sparkles className="w-5 h-5 text-purple-600" /> Assistente de
                Compras
              </h3>
              <button
            ...
            <p className="text-sm text-slate-600 dark:text-slate-300 mb-4">
              O que vamos comprar? Digite uma receita (ex: "Strogonoff para 4")
              ou um evento (ex: "Churrasco fim de semana") e eu gero a lista.
            </p>
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <CurrencyInput
                      placeholder="0,00"
                      value={
                        !editingItem.referencePrice
                          ? ""
                          : editingItem.referencePrice.toString()
                      }
                      onChangeValue={(val) =>
                        setEditingItem({
                          ...editingItem,
                          referencePrice: parseFloat(val) || 0,
                        })
                      }
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <CurrencyInput
                      placeholder="0,00"
                      value={
                        editingItem.actualPrice === 0
                          ? ""
                          : editingItem.actualPrice.toString()
                      }
                      onChangeValue={(val) =>
                        setEditingItem({
                          ...editingItem,
                          actualPrice: parseFloat(val) || 0,
                        })
                      }
        ...
        <div className="flex flex-col md:flex-row md:justify-between md:items-center mb-4 gap-4">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            >
              ...
              filteredItems.every((i) => i.isChecked)
                ? "Desmarcar Tudo"
                : "Selecionar Tudo"}
            </button>
            <button
              onClick={() => {
                if (
                  window.confirm(
                    "Tem certeza que deseja limpar a lista deste mês?",
                  )
                ) {
        ...
        <div className="bg-white dark:bg-slate-800 p-4 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700">
          <div className="flex justify-between items-end mb-2">
            ...
            <div className="text-right">
              <p
                className="text-[10px] font-bold uppercase text-slate-400 tracking-wider mb-1 flex items-center justify-end gap-1 cursor-pointer hover:text-indigo-500"
                onClick={() => {
                  setIsEditingBudget(true);
                  setTempBudget(shoppingBudget.toString());
                }}
              >
              ...
              {isEditingBudget ? (
                ...
                <p
                  className="text-xl font-bold text-slate-600 dark:text-slate-400 cursor-pointer hover:text-indigo-500"
                  onClick={() => {
                    setIsEditingBudget(true);
                    setTempBudget(shoppingBudget.toString());
                  }}
        ...
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
          <div className="bg-white dark:bg-slate-800 rounded-xl shadow-2xl w-full max-w-lg p-6 border border-slate-200 dark:border-slate-700 animate-scale-in">
            ...
            >
              <div>
                <label className="text-xs font-bold text-slate-500 uppercase block mb-1">
                  Buscar e Selecionar Produto do Catálogo
                </label>
                <div className="relative">
                  <div className="relative z-20">
                    <input
                      ...
                      className="w-full p-3 pr-10 rounded-xl border border-slate-300 dark:border-slate-600 dark:bg-slate-700 dark:text-white outline-none focus:ring-2 focus:ring-indigo-500 shadow-sm text-sm font-semibold"
                      autoComplete="off"
                      required
                    />
                  ...
                  {showSuggestions && (
                    <>
                      ...
                      />
                      <div className="absolute left-0 right-0 mt-1 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl shadow-2xl max-h-60 overflow-y-auto z-30 divide-y divide-slate-100 dark:divide-slate-700/60 animate-fade-in">
                        {registeredProducts.length === 0 && !productSearch.trim() ? (
                          <div className="p-4 text-sm text-slate-500 dark:text-slate-400 text-center">
                            Nenhum produto cadastrado no catálogo.
                            <p className="text-xs text-indigo-500 dark:text-indigo-400 mt-1.5 font-bold">
                              Digite o nome do alimento acima para cadastrá-lo de forma simplificada!
                            </p>
                          </div>
                        ) : filteredSuggestions.length === 0 ? (
                          <div className="p-4 text-sm text-slate-500 dark:text-slate-400 text-center flex flex-col gap-3">
                            <div>Nenhum produto cadastrado com este nome.</div>
                            {productSearch.trim() && (
                              <button
                                type="button"
                                onClick={handleQuickRegister}
                                className="w-full py-2.5 px-4 bg-indigo-600 hover:bg-indigo-700 text-white font-black text-xs rounded-lg transition-colors cursor-pointer shadow-sm flex items-center justify-center gap-1.5"
                              >
                                <Plus className="w-4 h-4" />
                                Cadastrar "{productSearch.trim()}" no Catálogo
                              </button>
                            )}
                          </div>
                        ) : (
                          <>
                            ...
                            {productSearch.trim() && !filteredSuggestions.some(p => p.name.toLowerCase() === productSearch.toLowerCase().trim()) && (
                              <button
                                type="button"
                                onClick={handleQuickRegister}
                                className="w-full text-left px-4 py-3 bg-indigo-50 dark:bg-indigo-950/30 hover:bg-indigo-100 dark:hover:bg-indigo-900/40 text-indigo-700 dark:text-indigo-300 flex items-center justify-between transition-colors border-t border-slate-100 dark:border-slate-700/60 font-bold text-xs cursor-pointer"
                              >
                                <div className="flex items-center gap-1.5">
                                  <Plus className="w-4 h-4 text-indigo-500" />
                                  <span>Cadastrar "{productSearch.trim()}" no catálogo</span>
                                </div>
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        value={newItemPrice}
                        onChange={(e) =>
                          setNewItemPrice(formatCurrencyInput(e.target.value))
                        }
                ...
                <div>
                  ...
                  <div className="flex flex-col gap-2">
                    ...
                    {repeatDates.map((date, idx) => (
                      <div key={idx} className="flex gap-2 items-center">
                        ...
                        <button
                          type="button"
                          onClick={() => {
                            const newDates = repeatDates.filter(
                              (_, i) => i !== idx,
                            );
                            setRepeatDates(newDates);
                          }}
                    ...
                    <button
                      type="button"
                      onClick={() =>
                        setRepeatDates([...repeatDates, newItemDate])
                      }
                      className="text-xs text-indigo-600 hover:text-indigo-800 font-bold flex items-center gap-1 mt-1"
                    >
      ...
      <div className="space-y-6">
        {filteredItems.length === 0 ? (
                            ...
                            <div className="flex items-center gap-3 flex-1 min-w-0">
                              ...
                              <div className="flex flex-col min-w-0">
                                ...
                                <div className="flex items-center gap-2 flex-wrap text-xs">
                                  {product.observation && (
                                    <span
                                      className={`truncate ${product.isChecked ? "text-slate-400/70" : "text-slate-500 dark:text-slate-400"}`}
                                    >
                                      {product.observation}
                                    </span>
                                  )}
          ...
          SHOPPING_CATEGORIES.map((category) => {
            ...
            const groupedByDate = categoryItems.reduce(
              (acc, item) => {
              ...
              >
                <div className="bg-slate-50 dark:bg-slate-900/50 px-4 py-2 border-b border-slate-100 dark:border-slate-700 flex justify-between items-center flex-wrap gap-2">
                  ...
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    {catSpentTotal > 0 && (
                      <span className="text-[10px] font-bold bg-emerald-50 dark:bg-emerald-950/30 text-emerald-600 dark:text-emerald-400 px-2 py-0.5 rounded border border-emerald-200/30 dark:border-emerald-900/30">
                        Carrinho: {formatValue(catSpentTotal)}
                      </span>
                    )}
                            ...
                            <div className="flex items-center gap-3 flex-1 min-w-0">
                              ...
                              <div className="flex flex-col min-w-0">
                                ...
                                {(() => {
                                  ...
                                  return (
                                    <div className="mt-1 mb-1.5 flex items-center gap-1.5 flex-wrap">
                                      <span className={`px-2 py-0.5 rounded text-[10px] font-black uppercase tracking-wide flex items-center gap-1 ${stockStatus.classes}`}>
                                        {stockStatus.label === "Diminuir estoque" && <TrendingDown className="w-3 h-3" />}
                                        {stockStatus.label === "Aumentar estoque" && <AlertTriangle className="w-3 h-3" />}
                                        {stockStatus.label === "Estoque está bom" && <Check className="w-3 h-3" />}
                                        {stockStatus.label}
                                      </span>
                                      <span className="text-[10px] font-medium text-slate-500">
                                        {stockStatus.message}
                                      </span>
                                      {stockStatus.label === "Diminuir estoque" && !item.isChecked && (
                                        <button
                                          type="button"
                                          onClick={() => {
                                            onUpdate(item.id, { quantity: stockStatus.suggestedQty });
                                          }}
                                          className="text-[10px] font-bold text-amber-700 hover:text-amber-800 bg-amber-50 hover:bg-amber-100 dark:bg-amber-950/20 dark:hover:bg-amber-950/40 px-1.5 py-0.5 rounded border border-amber-200 dark:border-amber-900/30 transition-colors cursor-pointer"
                                          title="Clique para aplicar a redução sugerida (-30%) no carrinho"
                                        >
                                          Aplicar redução (-30%): {stockStatus.suggestedQty} {stockStatus.unit}
                                        </button>
                                      )}
                                    </div>
                                  );
                                ...
                                <div className="flex items-center gap-2 flex-wrap text-xs">
                                  {item.purchaseDate && (
                                    <span
                                      className={`flex items-center gap-1 ${item.isChecked ? "text-slate-400/70" : "text-indigo-500 font-medium"}`}
                                    >
                                  ...
                                  {item.referencePrice !== undefined &&
                                    item.referencePrice > 0 && (
                                      <span
                                        className={`flex items-center gap-1 font-semibold text-[10px] px-1.5 py-0.5 rounded border ${
                                          item.isChecked
                                            ? "bg-slate-100/50 dark:bg-slate-800/50 text-slate-400 border-slate-200/50 dark:border-slate-700/50"
                                            : "bg-indigo-50/50 dark:bg-indigo-950/20 text-indigo-600 dark:text-indigo-400 border-indigo-100/30 dark:border-indigo-900/10"
                                        }`}
                                      >
                                        Ref: {formatValue(item.referencePrice)}
                                        {item.quantity > 1 && (
                                  ...
                                  {item.observation && (
                                    <span
                                      className={`truncate ${item.isChecked ? "text-slate-400/70" : "text-slate-500 dark:text-slate-400"}`}
                                    >
                                      {item.observation}
                                    </span>
                                  )}
